...
def test_sql_intent_rejects_product_sales_question_without_sales_or_region():
    ...
    assert not service._sql_matches_question_intent(
        'SELECT "dbo_products"."products" AS "products" FROM "dbo_products"',
...
def test_sql_intent_accepts_joined_product_sales_by_region_answer():
    service = AskService.__new__(AskService)

    assert service._sql_matches_question_intent(
        (
            'SELECT "dbo_products"."ProductName" AS "ProductName", '
            '"dbo_regions"."Region" AS "Region", '
            'SUM("dbo_order_lines"."SalesValue") AS "TotalSalesValue" '
            'FROM "dbo_order_lines" '
            'JOIN "dbo_products" ON "dbo_order_lines"."ProductId" = "dbo_products"."ProductId" '
            'JOIN "dbo_regions" ON "dbo_order_lines"."RegionId" = "dbo_regions"."RegionId" '
            'GROUP BY "dbo_products"."ProductName", "dbo_regions"."Region"'
        ),
        "Which products have the highest sales by region?",
        [
            {
                "name": "dbo_order_lines",
                "columns": [
                    {"name": "ProductId", "type": "VARCHAR"},
                    {"name": "RegionId", "type": "VARCHAR"},
                    {"name": "SalesValue", "type": "FLOAT"},
                ],
            },
            {
...
def test_prune_sql_generation_context_keeps_multiple_join_tables():
    service = AskService.__new__(AskService)
    table_ddls = [
        """
        CREATE TABLE dbo_Products (
        ...
        """,
        """
        CREATE TABLE dbo_Regions (
          RegionId VARCHAR,
          Region VARCHAR
        );
        """,
        """
        CREATE TABLE dbo_OrderLines (
          OrderLineId VARCHAR,
          ProductId VARCHAR,
          RegionId VARCHAR,
          SalesValue FLOAT,
          FOREIGN KEY (ProductId) REFERENCES dbo_Products(ProductId),
          FOREIGN KEY (RegionId) REFERENCES dbo_Regions(RegionId)
        );
        """,
        """
        CREATE TABLE dbo_Unrelated (
          id VARCHAR,
          notes VARCHAR
        );
        """,
    ...
    documents = [
        {"table_name": "dbo_Products", "table_ddl": table_ddls[0]},
        {"table_name": "dbo_Regions", "table_ddl": table_ddls[1]},
        {"table_name": "dbo_OrderLines", "table_ddl": table_ddls[2]},
        {"table_name": "dbo_Unrelated", "table_ddl": table_ddls[3]},
    ]
    ...
    assert table_names == ["dbo_Products", "dbo_Regions", "dbo_OrderLines"]
