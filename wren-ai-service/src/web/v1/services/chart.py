    ...
    async def chart(
        ...
        **kwargs,
        ...
        try:
            ...
            local_chart_result = build_fallback_chart_result(
                chart_request.query,
                sql_data,
                chart_request.remove_data_from_chart_schema,
            )
            if local_chart_result.get("chart_schema"):
                self._chart_results[query_id] = ChartResultResponse(
                    status="finished",
                    response=ChartResult(**local_chart_result),
                    trace_id=trace_id,
                )
                results["chart_result"] = local_chart_result
                return results
