...
4. Generate a concise and clear answer in string format and a reasoning process in string format to the user's question based on the data, sql and sql summary.
...
def prompt(
    ...
    logger.debug(f"sql summary: {sql_summary}")
...
if __name__ == "__main__":
    ...
    pipeline.visualize("query", "SELECT * FROM table_name", "summary", "English")
    async_validate(
        lambda: pipeline.run("query", "SELECT * FROM table_name", "summary", "English")
    )
