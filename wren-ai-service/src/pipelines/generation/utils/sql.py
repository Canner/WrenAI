    ...
    for start, end in _find_select_list_spans(sql):
        for item in _split_top_level_select_items(sql[start:end]):
            ...
            if not alias_match:
                continue
            expression = item[: alias_match.start()].strip()
            alias = _normalize_sql_identifier(alias_match.group("alias"))
            if expression and alias:
                aliases.add(alias.lower())
    return aliases


def _find_invalid_unqualified_column_references_for_single_table(
    sql: str,
    table_name: str,
    valid_columns: set[str],
    valid_compact_columns: set[str],
    aliases: dict[str, str],
) -> list[str]:
    table_aliases = {
        alias.lower()
        for alias, alias_table in aliases.items()
        if str(alias_table).lower() == str(table_name).lower()
    }
    table_aliases.update(
        suffix.lower() for suffix in _table_reference_suffixes(str(table_name))
    )
    projection_aliases = _extract_projection_aliases(sql)
    sql_without_strings = re.sub(r"'(?:''|[^'])*'", "''", sql or "")
    invalid_references: list[str] = []

    identifier_pattern = re.compile(
        r'"(?P<quoted>[^"]+)"|`(?P<backticked>[^`]+)`|\[(?P<bracketed>[^\]]+)\]|(?P<bare>\b[A-Za-z_][A-Za-z0-9_$]*\b)'
    )
    for match in identifier_pattern.finditer(sql_without_strings):
        identifier = (
            match.group("quoted")
            or match.group("backticked")
            or match.group("bracketed")
            or match.group("bare")
            or ""
        )
        ...
        if (
            normalized_identifier in _SQL_IDENTIFIER_KEYWORDS
            or normalized_identifier in table_aliases
            or normalized_identifier in projection_aliases
        ):
            continue
        if (
            normalized_identifier in valid_columns
            or _compact_sql_identifier(identifier) in valid_compact_columns
        ):
            continue
        if identifier not in invalid_references:
            invalid_references.append(identifier)
...
def find_invalid_column_references(
    ...
    if len(referenced_tables) == 1:
        ...
        invalid_references.extend(
            _find_invalid_unqualified_column_references_for_single_table(
                sql,
                str(table_name),
                valid_columns,
                valid_compact_columns,
