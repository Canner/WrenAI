...
_UNGROUNDED_REASONING_MESSAGE = (
    "1. **Schema-grounded plan required**: The reasoning plan contained SQL "
    "examples, speculative language, or unsupported schema identifiers, so it "
    "was rejected. SQL generation will rely only on verified schema objects "
    "from the active deployment."
)
_REASONING_SQL_SNIPPET = re.compile(
    r"```|\bSELECT\b[\s\S]{0,500}\bFROM\b|\bWITH\b[\s\S]{0,500}\bSELECT\b|"
    r"\b(?:INSERT|UPDATE|DELETE)\b|\bSQL\s+Query\b|"
    r"\bexample\s+(?:using\s+)?SQL\b",
    re.IGNORECASE,
)
_REASONING_SPECULATIVE_LANGUAGE = re.compile(
    r"\b(?:assum(?:e|ing|ed)|likely|probably|typically|placeholder|"
    r"not\s+explicitly|generic\s+name|example)\b",
    re.IGNORECASE,
)
_REASONING_TABLE_REFERENCE = re.compile(
    r"\btable:\s*(?:`(?P<quoted>[^`]+)`|(?P<bare>\"[^\"]+\"|\[[^\]]+\]|[A-Za-z_][A-Za-z0-9_$.\-]*))",
    re.IGNORECASE,
)
_REASONING_COLUMN_REFERENCE = re.compile(
    r"\bcolumn:\s*(?:`(?P<quoted>[^`]+)`|(?P<bare>\"[^\"]+\"|\[[^\]]+\]|[A-Za-z_][A-Za-z0-9_$.\-]*))",
    re.IGNORECASE,
)


def _clean_reasoning_identifier(identifier: str) -> str:
    return _unquote_identifier(identifier.strip().strip(" .:;"))


def _reasoning_reference(match: re.Match) -> str:
    return match.group("quoted") or match.group("bare") or ""


def sanitize_sql_generation_reasoning(
    reasoning: str | None,
    ...
    if _REASONING_SQL_SNIPPET.search(reasoning):
        return _UNGROUNDED_REASONING_MESSAGE

    if _REASONING_SPECULATIVE_LANGUAGE.search(reasoning):
        return _UNGROUNDED_REASONING_MESSAGE

    for match in _REASONING_TABLE_REFERENCE.finditer(reasoning):
        relation = _clean_reasoning_identifier(_reasoning_reference(match))
        if relation and relation not in schema_index:
            return _UNGROUNDED_REASONING_MESSAGE

    for match in _REASONING_COLUMN_REFERENCE.finditer(reasoning):
        qualified_column = _clean_reasoning_identifier(_reasoning_reference(match))
        if "." not in qualified_column:
            return _UNGROUNDED_REASONING_MESSAGE

        relation, column = qualified_column.rsplit(".", 1)
        relation = _clean_reasoning_identifier(relation)
        column = _clean_reasoning_identifier(column)
        valid_columns = schema_index.get(relation)
        if valid_columns is None and relation in schema_index:
            continue
        if not valid_columns or column not in valid_columns:
            return _UNGROUNDED_REASONING_MESSAGE
