...
def construct_semantic_schema_contract(
    ...
    lines = [
        ...
        "Use this contract as the primary source of truth for SQL generation.",
        "Generate SQL only from schema objects listed here and in the retrieved DATABASE SCHEMA.",
        "Do not infer alternative tables, columns, joins, metrics, filters, or time fields independently.",
    ]
    ...
    for label, key in (
        ...
        values = _semantic_analysis_items(semantic_analysis, key)
        if values:
            lines.append(f"{label}: {', '.join(values)}")

    mappings = _semantic_analysis_dict_items(semantic_analysis, "concept_mappings")
    if mappings:
        ...
        for mapping in mappings:
            ...
            lines.append(
                ...
                f"({_mapping_concept_type(mapping) or 'concept'}, {required}{confidence_text}) "
                f"-> {', '.join(schema_objects) or 'NO_MAPPING'}."
            )
    ...
    if interpretations:
        ...
        for interpretation in interpretations:
            ...
            schema_text = (
                ", ".join(str(item).strip() for item in schema_objects if item)
                if isinstance(schema_objects, list)
                else ""
            )
            lines.append(f"- {description} ({selected}). Objects: {schema_text}")
    ...
    lines.append(
        "Validation requirement: every required mapped schema object must appear in SQL when it represents an entity, identifier, dimension, metric, filter, time field, relationship, or requested output. Ranking/sorting/aggregation concepts must appear as SQL shape."
    )
    return "\n".join(lines)
...
def get_schema_intent_analysis_error(
    ...
    if not _has_semantic_analysis(semantic_analysis):
        return None
    missing = _semantic_analysis_items(semantic_analysis, "missing_requirements")
    if missing:
        return (
            "The active datasource schema does not expose the information needed "
            f"to answer the request: {', '.join(missing)}. I cannot generate unrelated SQL."
        )
    ambiguous = _semantic_analysis_items(semantic_analysis, "ambiguous_requirements")
    if ambiguous:
        return (
            "The request has multiple equally plausible schema interpretations: "
            f"{', '.join(ambiguous)}. Please clarify which one to use."
        )
    candidates = _semantic_analysis_dict_items(
        semantic_analysis, "candidate_schema_scores"
    )
    if candidates and not any(candidate.get("is_complete") is True for candidate in candidates):
        missing_concepts = []
        for candidate in candidates[:3]:
            missing_value = candidate.get("missing_concepts")
            if isinstance(missing_value, list) and missing_value:
                missing_concepts.append(
                    f"{candidate.get('candidate_id') or 'candidate'}: "
                    + ", ".join(str(item) for item in missing_value if item)
                )
        if missing_concepts:
            return (
                "Semantic schema retrieval did not find a complete mapping for "
                f"the request. Missing concepts: {'; '.join(missing_concepts)}."
            )
    if semantic_analysis.get("is_fully_supported") is False:
        reason = str(semantic_analysis.get("support_reasoning") or "").strip()
        return reason or (
            "The selected schema does not fully support every required component of the request."
        )
    schema_bound_types = {
        "dimension",
        "entity",
    ...
    unsupported = []
    for mapping in _semantic_analysis_dict_items(semantic_analysis, "concept_mappings"):
        if mapping.get("required_in_sql") is False:
            continue
        if _mapping_concept_type(mapping) not in schema_bound_types:
            continue
        if not _mapping_schema_objects(mapping):
            unsupported.append(_mapping_request_concept(mapping))
    if unsupported:
        return (
            "The semantic contract did not map required request concepts to active "
            f"schema objects: {', '.join(unsupported)}."
        )
    return None
...
def _extract_placeholder_schema_references(sql: str) -> list[str]:
    ...
    placeholder_names = re.findall(
        r"\b(?:table|column|schema|database|field|metric|dimension|date|amount|entity)_?name\b",
        sql or "",
        flags=re.IGNORECASE,
    )
    return sorted(
        {
            str(item).strip()
            for item in [*placeholders, *placeholder_names]
            if item is not None and str(item).strip()
        }
    )
...
def _sql_references_table(sql: str, table_name: str) -> bool:
    normalized_table = _compact_sql_identifier(table_name)
    if not normalized_table:
        return False
    table_suffixes = {
        _compact_sql_identifier(suffix)
        for suffix in _table_reference_suffixes(table_name)
    }
    table_suffixes.add(normalized_table)
    for referenced_table in extract_sql_table_references(sql or ""):
        normalized_reference = _compact_sql_identifier(referenced_table)
        if not normalized_reference:
            continue
        if normalized_reference in table_suffixes:
            return True
        if any(
            normalized_reference.endswith(suffix) or suffix.endswith(normalized_reference)
...
def _sql_references_schema_object(
    ...
    table = ".".join(parts[:-1])
    column = parts[-1]
    return _sql_references_table(sql, table) and bool(
        re.search(
...
def _sql_is_plain_count(sql: str) -> bool:
    return bool(
        re.search(r"\bCOUNT\s*\(\s*\*\s*\)", sql or "", flags=re.IGNORECASE)
    ) and not re.search(
        r"\b(?:SUM|AVG|MIN|MAX)\s*\(", sql or "", flags=re.IGNORECASE
    )


def _semantic_requests_record_count(semantic_analysis: dict[str, Any]) -> bool:
    intent = str(semantic_analysis.get("analytical_intent") or "").lower()
    if intent == "record_count":
        return True
    text = " ".join(
        item
        for key in ("metrics", "aggregations", "requested_output")
        for item in _semantic_analysis_items(semantic_analysis, key)
    ).lower()
    return bool(re.search(r"\b(?:count|number of|row count|record count)\b", text))
...
def validate_sql_intent_alignment(
    ...
    requests_count = _semantic_requests_record_count(semantic_analysis)
    mappings = _semantic_analysis_dict_items(semantic_analysis, "concept_mappings")
    schema_bound_types = {
        "dimension",
        "entity",
    ...
    for mapping in mappings:
        ...
        if concept_type not in schema_bound_types:
            continue
        schema_objects = _mapping_schema_objects(mapping)
    ...
    if _semantic_analysis_items(semantic_analysis, "ranking") and not re.search(
        r"\bORDER\s+BY\b.*\b(?:LIMIT|FETCH\s+FIRST|TOP\s*(?:\(\s*)?\d+)\b|\b(?:LIMIT|FETCH\s+FIRST|TOP\s*(?:\(\s*)?\d+)\b.*\bORDER\s+BY\b",
        sql or "",
        flags=re.IGNORECASE | re.DOTALL,
    ):
        return "Generated SQL does not include sorting and limiting logic required by the ranking intent."
    if _semantic_analysis_items(semantic_analysis, "sorting") and not re.search(
        r"\bORDER\s+BY\b", sql or "", flags=re.IGNORECASE
    ):
        return "Generated SQL does not include sorting logic required by the semantic intent."
    if _semantic_analysis_items(semantic_analysis, "aggregations") and not re.search(
        r"\b(?:COUNT|SUM|AVG|MIN|MAX)\s*\(", sql or "", flags=re.IGNORECASE
    ):
        return "Generated SQL does not include the aggregation required by the semantic intent."
    return None
