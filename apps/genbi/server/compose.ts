...
export function composeSetupPrompt(
  ...
  const credentialBoundary =
    "Never ask for or print credential values (passwords, API keys, tokens, connection secrets, etc); " +
  ...
  // no-redirection/cwd-field rule (the sandbox constraint itself never changes).
  ...
  // says only what's true, and tells the agent about the exec action's optional `cwd` field —
  // the sanctioned way to reach a specific directory, instead of `cd`-chaining.
  const bashDisciplineFor = (skillName: string, disciplineOptions?: { readonly skipSkillFetch?: boolean }): string => {
    const sandboxRule =
      ...
      "To run a command inside a specific directory (e.g. the project directory), pass the exec action's optional cwd field rather than 'cd <dir> && ...' chaining; " +
      "pipes and ||/&& chaining are not specifically blocked, but are unnecessary once cwd is available and should be avoided.";
  ...
  if (stepKey === "connect") {
    return (
      ...
      `under the workspace root "${workspaceRoot}". The wren CLI is already installed and on PATH, with the "${sourceType}" datasource extra it needs ` +
      `already available — there is nothing to install, so skip the skill's Preflight section entirely and skip Step 2's "pip install" sub-step; ` +
      ...
      `for the "${sourceType}" connection's required variables — do not fill in any values. Then stop and wait for the user to fill in the .env file. ` +
  ...
  if (stepKey === "context") {
    ...
    const opening = options?.schemaDiscoveryRecovery
      ...
      : options?.resumeFromDisk
        ...
        : `Follow the wren generate-mdl skill (and, optionally, the enrich-context skill) to generate the MDL for the wren project "${projectName}" ` +
          `at "${projectDir}". `;
    ...
    const generateMdlNote =
      options?.resumeFromDisk === true || options?.resumeSession === true
    ...
    const schemaDiscoveryRecovery = options?.schemaDiscoveryRecovery
    ...
    return (
      ...
      `Run every wren CLI command in this step — schema-introspection commands, "wren context build", and "wren context validate" — with the exec action's cwd field set to "${projectDir}", never the workspace root: the wren CLI's .env auto-discovery only checks the process's current working directory and completely ignores --path, so a command run from the wrong cwd will report a connection variable such as DUCKDB_URL as unset even though "${projectDir}/.env" is filled in correctly — that is NOT a real connection failure, it means you forgot to set cwd. ` +
      `You MUST run "wren context build" (this compiles the model YAML into target/mdl.json — the build must succeed and produce at least one model); ` +
      `run "wren context build" BEFORE "wren context validate" (validate reads the already-built target/mdl.json, so building first is required). ` +
      ...
      `Never hand-write a placeholder, seed, sample, or otherwise invented model, cube, or metadata file just to make "wren context build" succeed — every model, column, and measure in the MDL must come from something you actually introspected from the connected warehouse, never from imagination. If schema introspection yields no tables, or a connection variable still reports as unset after you've set cwd as instructed above, STOP: do not write any model/cube/metadata content, and end the turn with SETUP_STATUS: error (or SETUP_STATUS: needs_input if the user needs to act, e.g. fix a bad credential) explaining exactly why — never fabricate content just to satisfy the build. ` +
      `Also define at least ONE measure using the enrich-context skill (or by authoring the MDL directly) — a simple aggregate such as a SUM or COUNT over a numeric column is enough; the genbi profile requires at least one declared measure to bind. Rebuild after adding it. ` +
      `As soon as "wren context build" succeeds and target/mdl.json contains at least one model AND at least one measure, STOP: report the model and measure counts and SETUP_STATUS: ok in your final message. Do NOT run further validate/query/cleanup loops after a successful build. ` +
      `${bashDisciplineFor("generate-mdl", { skipSkillFetch: options?.resumeFromDisk === true || options?.resumeSession === true })} ${terminalContract}`
    );
  }

  return (
    `The user has filled in the .env file for the wren project "${projectName}" at "${projectDir}", but no connection profile exists for it yet. ` +
    ...
    `Write that scratch file with the write action with cwd set to "${projectDir}" and path set exactly to "conn.profile.yml"; do not prepend the workspace path or project name to that write path, because cwd already selects the project directory. ` +
    ...
    `"wren profile add" has no project-path flag of its own, so run it with the exec action's cwd field set to "${projectDir}" — never "cd ${projectDir} && ..." chaining — so it discovers the right project. ` +
    `Only once that command succeeds should you consider the connection validated, and report whether it succeeded. ` +
    `Only if validation genuinely succeeds, create the sentinel with the write action's cwd set to "${projectDir}" and path set exactly to ".wren-validated" (empty content, no shell redirection) ` +
