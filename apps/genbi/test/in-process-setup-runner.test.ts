...
// Same mocking approach as test/dispatched-setup-runner.test.ts: mock the one
...
describe("InProcessSetupRunner (the real production in-process setup runner, not a stub)", () => {
  it("a subscription authChoice throws WITHOUT dispatching anything", async () => {
    const { InProcessSetupRunner } = await import("../harness/setup/runner.js");
    const runner = new InProcessSetupRunner({ irPath: "/fixture/genbi-setup/ir.golden.json", warbleBin: EXISTING_FILE });

    await expect(
      runner.run({
        prompt: "scaffold a new project",
        workspaceRoot: "/tmp/wren-harness-in-process-setup-runner-test",
  ...
  it("an api-key dispatch runs the compiled bundle's agent for real (via a scripted mock model) and returns the produced artifact as finalText/sessionId:null", async () => {
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "wren-harness-in-process-setup-runner-ws-"));
    try {
      respondWithBundle(
      ...
      // InProcessSetupRunner builds the model binding itself from `authChoice` via
  ...
  it("uses the live persisted strong-tier AdapterSpec instead of the boot-time auth/model defaults", async () => {
    ...
    try {
      ...
      const { InProcessSetupRunner } = await import("../harness/setup/runner.js");
      const runner = new InProcessSetupRunner({
  ...
  it("actually reaches the real setup_execution native tool: a scripted tool call executes a real shell command scoped to workspaceRoot, and writes a real file within scope", async () => {
    ...
    try {
      ...
      // proof `InProcessSetupRunner` wires a real ExecutionEnv/policy/native-tool
  ...
  it("effectiveMaxTurns() reports exactly the budget run() dispatches with — build_context gets DEFAULT_SETUP_MAX_TURNS by default, connect_source stays undefined, and an explicit override always wins", async () => {
    const { InProcessSetupRunner, DEFAULT_SETUP_MAX_TURNS } = await import("../harness/setup/runner.js");

    const defaultRunner = new InProcessSetupRunner({ irPath: "/fixture/genbi-setup/ir.golden.json", warbleBin: EXISTING_FILE });
    ...
    const overriddenRunner = new InProcessSetupRunner({
      irPath: "/fixture/genbi-setup/ir.golden.json",
      warbleBin: EXISTING_FILE,
      maxTurns: 2,
    });
  ...
  it("run() actually dispatches the effectiveMaxTurns() value as ExecuteAgentContext.maxSteps — a scripted model that never finishes on its own is cut off exactly there, as StepBudgetExhaustedError", async () => {
    const workspaceRoot = await mkdtemp(path.join(os.tmpdir(), "wren-harness-in-process-setup-runner-budget-"));
    try {
      respondWithBundle(buildSyntheticBundle({ tools: [{ name: "setup_execution", source: "native" }] }));

      const { InProcessSetupRunner, StepBudgetExhaustedError } = await import("../harness/setup/runner.js");
      const runner = new InProcessSetupRunner({ irPath: "/fixture/genbi-setup/ir.golden.json", warbleBin: EXISTING_FILE, maxTurns: 2 });
      ...
      // see InProcessSetupRunner.run()'s catch block.
