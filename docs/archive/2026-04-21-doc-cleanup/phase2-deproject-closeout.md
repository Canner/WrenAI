...
- repository / service / 历史 resolver / utils 的 bridge-only helper 已继续收口
- route / controller / 历史 resolver / runtime helper 不再新增新的 `getCurrentProject()` 或 pipeline legacy parameter
...
- `历史 project resolver 层 / runtimeScope / persistedRuntimeIdentity` 已继续复用 shared canonical helper，并把 legacy project bridge 读取/选择器构造进一步收口到内部 helper
- `modelService / 历史 modelResolver 层` 第二轮收口已完成：modelResolver 直接复用 scope->canonical helper，modelService 将多处 project-domain 参数名进一步明确为 legacy project bridge 语义，热点继续下降
- 重新扫描后的热点命中（含测试）主要集中在：
  ...
  - `wren-ui/src/server/services/mdlService.ts`
  - `wren-ui/src/server/services/dashboardService.ts`
  - `（历史）resolver 层 modelResolver.ts`
- 这些热点里的剩余命中，已经越来越集中到：
  1. 真实领域主键 / 持久化字段
...
2. **已收口但仍有文本命中**
   ...
   - controller/历史 resolver/service 内部的 bridge-only 分支
3. **后续可继续收口**
...
#### `（历史）manager 层 dataSourceSchemaDetector.ts`
- 全部围绕真实 `project.id`
- 属于数据源探测/变更分析流程
...
#### `wren-ui/src/server/utils/persistedRuntimeIdentity.ts`
- `projectId` 是兼容桥字段定义的一部分
- 不能删除，只能继续强化 helper 语义

#### `（历史）resolver 层 projectResolver.ts`
...
#### `（历史）resolver 层 dashboardResolver.ts`
...
- 优先改 service / controller / 历史 resolver 内部私有方法
...
- `wren-ui` 的 repository / service / controller / 历史 resolver / utils 低风险命名收口已基本完成
...
1. 让 **runtime 主语义** 从 project-first 切到
   ...
   - `kbSnapshotId`
   - `deployHash`
2. 让 repository / service / controller / 历史 resolver / pipeline 主链不再依赖 `getCurrentProject()`
...
- `wren-ui/src/server/services/modelService.ts` → `69`
- `wren-ui/src/server/context/runtimeScope.ts` → `35`
- `wren-ui/src/server/utils/persistedRuntimeIdentity.ts` → `23`
- `（历史）resolver 层 modelResolver.ts` → `23`
- `wren-ui/src/server/services/mdlService.ts` → `21`
- `（历史）resolver 层 projectResolver.ts` → `17`
- `wren-ui/src/server/services/deployService.ts` → `11`
- `wren-ui/src/server/utils/dashboardRuntime.ts` → `10`
- `wren-ui/src/server/utils/runtimeExecutionContext.ts` → `6`
