...
WREN_AGENTIC_SYSTEM_BASE_URL = "http://localhost:5556"
...
def rerun_wren_engine(mdl_json: Dict, dataset_type: str, dataset: str):
    ...
    # wait for wren-agentic-system to restart
    time.sleep(5)
...
def generate_mdl_metadata(mdl_model_json: dict):
    ...
    generate_mdl_metadata_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/semantics-descriptions",
        json={
            "mdl": mdl_model_json,
...
def prepare_semantics(mdl_json: dict):
    semantics_preparation_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/semantics-preparations",
        json={
    ...
    while (
        ...
        semantics_preparation_status_response = requests.get(
            f'{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/semantics-preparations/{st.session_state['deployment_id']}/status'
        )
        st.session_state[
...
def ask(query: str, query_history: Optional[dict] = None):
    ...
    asks_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/asks",
        json={
            "query": query,
            "id": st.session_state["deployment_id"],
    ...
    while not asks_status or (
        ...
        asks_status_response = requests.get(
            f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/asks/{query_id}/result"
        )
...
def ask_details():
    asks_details_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/ask-details",
        json={
    ...
    while (
        ...
        asks_details_status_response = requests.get(
            f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/ask-details/{query_id}/result"
        )
...
def sql_explanation():
    sql_explanation_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/sql-explanations",
        json={
    ...
    while (
        ...
        sql_explanation_status_response = requests.get(
            f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/sql-explanations/{query_id}/result"
        )
...
def sql_regeneration(sql_regeneration_data: dict):
    sql_regeneration_response = requests.post(
        f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/sql-regenerations",
    ...
    while (
        ...
        sql_regeneration_status_response = requests.get(
            f"{WREN_AGENTIC_SYSTEM_BASE_URL}/v1/sql-regenerations/{query_id}/result"
        )
