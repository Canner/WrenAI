...
export class ProjectController {
  ...
  public async getSettings({ ctx }: { ctx: IContext }) {
    if (!ctx.runtimeScope) {
  ...
  public async getProjectRecommendationQuestions(
    ...
    if (!project) {
      ...
      await this.recordKnowledgeBaseReadAudit(ctx, {
        ...
        resourceId:
          knowledgeBase?.id || ctx.runtimeScope?.workspace?.id || null,
        payloadJson: {
  ...
  public async resetCurrentProject({ ctx }: { ctx: IContext }) {
    const [project, knowledgeBase] = await Promise.all([
      this.resolveActiveRuntimeProject(ctx),
      this.resolveActiveRuntimeKnowledgeBase(ctx),
    ]);
  ...
  public async getOnboardingStatus({ ctx }: { ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
    const knowledgeBase = await this.resolveActiveRuntimeKnowledgeBase(ctx);
    const project = await this.resolveActiveRuntimeProject(ctx);
    const sampleDataset =
  ...
  public async saveTables(
    ...
    try {
      ...
      void this.deploy(ctx, project).catch((error) => {
        logger.error(
  ...
  public async autoGenerateRelation({ ctx }: { ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
    const project = await this.resolveActiveRuntimeProject(ctx);
    const knowledgeBase = await this.resolveActiveRuntimeKnowledgeBase(ctx);
    if (!project) {
  ...
  public async saveRelations(
    ...
    try {
      const project = await this.getActiveRuntimeProjectOrThrow(ctx);
      await this.assertKnowledgeBaseWriteAccess(ctx);
      await this.ensureModelsBelongToActiveRuntime(
        ctx,
      ...
      const savedRelations = runtimeIdentity
        ...
        : await ctx.modelService.saveRelations(arg.data.relations);
      // async deploy
      void this.deploy(ctx, project).catch((error) => {
        logger.error(
          `Failed to deploy project ${project.id} after saving relations: ${error.message}`,
        );
      });
      ctx.telemetry.sendEvent(eventName, {
  ...
  public async getSchemaChange({ ctx }: { ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
    const project = await this.resolveActiveRuntimeProject(ctx);
    const knowledgeBase = await this.resolveActiveRuntimeKnowledgeBase(ctx);
    if (!project) {
  ...
  private async ensureModelsBelongToActiveRuntime(
    ctx: IContext,
    modelIds: number[],
    projectId: number,
  ) {
