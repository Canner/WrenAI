...
export class ModelController {
  ...
  public async createCalculatedField({
    ...
    await this.assertKnowledgeBaseWriteAccess(ctx);
    const { runtimeIdentity } = this.getRuntimeSelection(ctx);
    await this.ensureModelScope(ctx, data.modelId);
  ...
  public async checkModelSync({ ctx }: { ctx: IContext }) {
  ...
  public async deploy({
    ...
    if (!(allowInternalBypass && this.isInternalAiServiceRequest(ctx))) {
      await this.assertKnowledgeBaseWriteAccess(ctx);
    }
    const { runtimeIdentity } = this.getRuntimeSelection(ctx);
    const mdlResult =
  ...
  public async getMDL({ hash, ctx }: { hash: string; ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
  ...
  public async listModels({ ctx }: { ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
    const { runtimeIdentity } = this.getRuntimeSelection(ctx);
    const models =
  ...
  public async getModel({ modelId, ctx }: { modelId: number; ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
  ...
  private async handleCreateModel(
    ...
    this.validateTableExist(sourceTableName, connectionTables);
    this.validateColumnsExist(sourceTableName, fields, connectionTables);

    // create model
    const connectionTable = connectionTables.find(
      (table) => table.name === sourceTableName,
    );
    if (!connectionTable) {
      throw new Error('Table not found in the connection');
    }
    const properties = connectionTable?.properties;
    const modelValue = {
    ...
    // create columns
    const compactColumns = connectionTable.columns.filter((c) =>
  ...
  private async handleUpdateModel(
    ...
    const connectionTables =
      await ctx.projectService.getProjectConnectionTables(project);
    const model = await this.ensureModelScope(ctx, input.modelId);
    ...
    const sourceTableColumns =
      connectionTables.find((table) => table.name === sourceTableName)
        ?.columns ?? [];
  ...
  // list views
  public async listViews({ ctx }: { ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
    const { runtimeIdentity } = this.getRuntimeSelection(ctx);
    const views =
  ...
  public async getView({ viewId, ctx }: { viewId: number; ctx: IContext }) {
    await this.assertKnowledgeBaseReadAccess(ctx);
  ...
  public async validateView({ name, ctx }: { name: string; ctx: IContext }) {
    await this.assertKnowledgeBaseWriteAccess(ctx);
  ...
  // delete view
  public async deleteView({ viewId, ctx }: { viewId: number; ctx: IContext }) {
    await this.assertExecutableRuntimeScope(ctx);
    await this.assertKnowledgeBaseWriteAccess(ctx);
  ...
  private isInternalAiServiceRequest(ctx: IContext): boolean {
  ...
  private isInternalAiServicePreviewRequest(
    ...
    return this.isInternalAiServiceRequest(ctx);
  }
  ...
  private validateTableExist(
    ...
    if (!connectionTables.find((c) => c.name === tableName)) {
  ...
  private validateColumnsExist(
    ...
    const tableColumns = connectionTables.find(
      (c) => c.name === tableName,
    )?.columns;
