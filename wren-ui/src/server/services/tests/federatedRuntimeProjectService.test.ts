...
describe('FederatedRuntimeProjectService', () => {
  ...
  it('keeps non-federated runtime projects when connector count drops below federation threshold', async () => {
    knowledgeBaseRepository.findOneBy.mockResolvedValue({
      id: 'kb-1',
      workspaceId: 'ws-1',
      name: 'Sales KB',
    ...
    expect(deployLogRepository.deleteAllBy).not.toHaveBeenCalled();
    expect(projectRepository.deleteOne).not.toHaveBeenCalled();
    expect(knowledgeBaseRepository.updateOne).not.toHaveBeenCalledWith(
      'kb-1',
