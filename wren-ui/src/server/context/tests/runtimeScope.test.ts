...
describe('RuntimeScopeResolver', () => {
  ...
  it('reads explicit selector from request variables payload', async () => {
    kbSnapshotRepository.findOneBy.mockResolvedValueOnce({
      id: 'snap-vars',
  ...
  it('downgrades stale canonical snapshot selectors to draft knowledge-base scope when no deployment exists', async () => {
    kbSnapshotRepository.findOneBy.mockResolvedValueOnce({
      id: 'snap-stale',
    ...
    deployService.getDeploymentByRuntimeIdentity.mockResolvedValue(null);
    ...
    expect(result.project).toBeNull();
    expect(result.deployment).toBeNull();
    expect(result.kbSnapshot).toBeNull();
    expect(result.deployHash).toBeNull();
    expect(result.selector.workspaceId).toBe('ws-stale');
    expect(result.selector.knowledgeBaseId).toBe('kb-stale');
    expect(result.selector.kbSnapshotId).toBeNull();
