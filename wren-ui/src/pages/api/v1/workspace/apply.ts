  ...
  try {
    ...
    if (!canManageWorkspaceJoinFlow(workspace.kind)) {
      return res.status(403).json({
        error: 'Default workspace does not support join applications',
      });
    }

    const membership = await components.workspaceService.applyToWorkspace({
      workspaceId,
    ...
    return res.status(400).json({
      error: error?.message || 'Failed to submit workspace application',
    });
  }
}
