...
export default async function handler(
  ...
  try {
    ...
    if (req.method === 'GET') {
      const users = await components.scimService.listUsers(context);
      await recordScimReadAudit({
        auditEventRepository: components.auditEventRepository,
        req,
        context,
        resourceType: 'scim_user_collection',
        resourceId: context.provider.id,
        payloadJson: {
          count: users.length,
        },
      });
      return res.status(200).json(scimListResponse(users));
    }

    const user = await components.scimService.createUser(
      context,
