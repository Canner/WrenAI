import { SqlPairRepository } from './sqlPairRepository';
...
describe('SqlPairRepository runtime scope query', () => {
  it('treats projectId as fallback when canonical runtime scope exists', async () => {
    const { knex, builder } = buildKnexRows([]);
    const repository = new SqlPairRepository(knex as unknown as any);
