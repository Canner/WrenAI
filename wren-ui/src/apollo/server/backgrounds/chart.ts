...
const computeBackoffMs = (retryCount: number) => {
  return Math.min(BASE_BACKOFF_MS * 2 ** Math.max(retryCount - 1, 0), MAX_BACKOFF_MS);
};
...
abstract class BaseChartBackgroundTracker {
  ...
  protected abstract fetchResult(queryId: string): ReturnType<IWrenAIAdaptor['getChartResult']>;
      ...
      const jobs = Object.values(this.tasks).map(
        (threadResponse) => async () => {
          ...
          const nextRetryAt = chartDetail.nextRetryAt
            ...
            : null;
          if (nextRetryAt && Number.isFinite(nextRetryAt) && nextRetryAt > Date.now()) {
            return;
          }
          ...
          try {
            const leaseClaim = await this.threadResponseRepository.claimChartPollingLease?.(
              threadResponse.id,
            ...
            const { canonicalChartSchema, canonicalizationVersion, renderHints, validationErrors } =
            ...
            const updatedChartDetail = {
              ...
              rawChartSchema:
                result?.response?.chartSchema || trackedChartDetail.rawChartSchema,
              chartSchema:
            ...
            if (isFinalized(result.status)) {
              ...
              if (result.status === ChartStatus.FINISHED) {
                this.telemetry.sendEvent(this.getTelemetryEvent(), eventProperties);
              } else {
  ...
  private async handleTaskFailure(threadResponse: ThreadResponse, error: unknown) {
    const chartDetail = threadResponse.chartDetail;
    if (!chartDetail?.queryId) {
      delete this.tasks[threadResponse.id];
      return;
