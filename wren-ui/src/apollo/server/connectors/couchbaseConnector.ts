...
export interface CouchbaseConstraintResponse {
...
export class CouchbaseConnector
  implements IConnector<CouchbaseColumnResponse, CouchbaseConstraintResponse>
{
  private config: CouchbaseConnectionConfig;
  ...
  public async connect(): Promise<boolean> {
    try {
      ...
      logger.error(`Error connecting to Couchbase: ${err}`);
      return false;
    }
  }

  public async listTables(options: CouchbaseListTableOptions) {
    const sql = `
      SELECT
        t.table_catalog,
        t.table_schema,
  ...
  public async listConstraints() {
    ...
    }) as CouchbaseConstraintResponse[];
    return constraints;
  }
  ...
  private transformColumnType(dataType: string) {
    ...
    // all possible types listed here: https://www.Couchbaseql.org/docs/current/datatype.html#DATATYPE-TABLE
