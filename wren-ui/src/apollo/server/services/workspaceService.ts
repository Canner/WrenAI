...
export class WorkspaceService implements IWorkspaceService {
  ...
  private assertActorAllowed({
    ...
    if (!decision.allowed) {
      const error = new Error(decision.reason || 'Permission denied') as Error & {
        statusCode?: number;
      };
  ...
  public async updateDefaultWorkspace(
    ...
    const actor = {
      ...
      permissionScopes: input.validatedSession.actorClaims.permissionScopes || [],
      isPlatformAdmin: Boolean(input.validatedSession.actorClaims.isPlatformAdmin),
      platformRoleKeys: input.validatedSession.actorClaims.platformRoleKeys || [],
      grantedActions: input.validatedSession.actorClaims.grantedActions || [],
      workspaceRoleSource: input.validatedSession.actorClaims.workspaceRoleSource,
