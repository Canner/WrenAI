...
describe('AskingService', () => {
  ...
  describe('SQL correction application', () => {
    test('updates the original response and clears stale downstream state', async () => {
      const service = Object.create(AskingService.prototype) as any;
      service.threadResponseRepository = {
        findOneBy: jest.fn().mockResolvedValue({
          ...
          threadId: 7,
          askingTaskId: 42,
          question: 'Show the corrected result',
          sql: 'SELECT 0',
          answerDetail: { status: 'FINISHED', content: 'stale answer' },
          chartDetail: { status: 'FINISHED', chartSchema: { stale: true } },
        }),
      ...
      service.projectService = {
        getCurrentProject: jest.fn().mockResolvedValue({ id: 1 }),
        getProjectById: jest.fn().mockResolvedValue({ id: 1 }),
      };
      service.deployService = {
        getLastDeployment: jest.fn().mockResolvedValue({ manifest: {} }),
      };
      service.queryService = {
        preview: jest.fn().mockResolvedValue({ columns: [], data: [] }),
      };
      service.askingTaskRepository = {
        updateOne: jest.fn().mockResolvedValue(undefined),
      };

      const response = await service.adjustThreadResponseWithSQL(11, {
        sql: 'SELECT 1',
      });

      expect(response.sql).toBe('SELECT 1');
      expect(service.threadResponseRepository.createOne).not.toHaveBeenCalled();
      expect(service.queryService.preview).toHaveBeenCalledWith('SELECT 1', {
        project: { id: 1 },
        manifest: {},
        modelingOnly: false,
        limit: 1,
      ...
      expect(service.askingTaskRepository.updateOne).toHaveBeenCalledWith(
        42,
        expect.objectContaining({
