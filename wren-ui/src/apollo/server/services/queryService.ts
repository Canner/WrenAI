...
const extractProjectionAliases = (sql: string) => {
  ...
  while ((match = selectPattern.exec(sql))) {
    ...
    splitTopLevelSqlList(body).forEach((item) => {
      const aliasMatch = item.match(
        new RegExp(
          String.raw`\s+(?:AS\s+)?(?<alias>${SQL_IDENTIFIER_PATTERN})\s*$`,
          'i',
        ),
      );
      if (!aliasMatch?.groups?.alias || aliasMatch.index === undefined) {
        return;
      }
      const expression = item.slice(0, aliasMatch.index).trim();
      if (expression) {
        aliases.add(
          normalizeSqlIdentifier(aliasMatch.groups.alias).toLowerCase(),
        );
      }
    });
  }
...
const findInvalidUnqualifiedColumnReferencesForSingleSchema = (
  sql: string,
  schema: ManifestModelSchema,
  aliases: Map<string, ManifestModelSchema>,
) => {
  const tableAliases = new Set<string>();
  aliases.forEach((aliasSchema, alias) => {
    if (aliasSchema === schema) {
      tableAliases.add(alias.toLowerCase());
    }
  });
  const projectionAliases = extractProjectionAliases(sql);
  const sqlWithoutStrings = sql.replace(/'(?:''|[^'])*'/g, "''");
  const invalidReferences: string[] = [];
  const identifierPattern =
    /"(?<quoted>[^"]+)"|`(?<backticked>[^`]+)`|\[(?<bracketed>[^\]]+)\]|(?<bare>\b[A-Za-z_][A-Za-z0-9_$]*\b)/g;

  let match: RegExpExecArray | null;
  while ((match = identifierPattern.exec(sqlWithoutStrings))) {
    const identifier =
      match.groups?.quoted ||
      match.groups?.backticked ||
    ...
    const before = sqlWithoutStrings.slice(0, match.index).trimEnd();
    const after = sqlWithoutStrings.slice(match.index + match[0].length).trimStart();
    ...
    if (
      SQL_IDENTIFIER_KEYWORDS.has(normalizedIdentifier) ||
      tableAliases.has(normalizedIdentifier) ||
      projectionAliases.has(normalizedIdentifier)
    ) {
      continue;
    }
    if (
      schema.columns.has(normalizedIdentifier) ||
      schema.columns.has(compactSqlIdentifier(identifier))
    ) {
      continue;
    }
    if (!invalidReferences.includes(identifier)) {
...
const findSqlReferenceValidationErrors = (
  ...
  if (activeSchemas.length === 1) {
    ...
    errors.push(
      ...findInvalidUnqualifiedColumnReferencesForSingleSchema(
        sql,
        schema,
        aliases,
      ),
