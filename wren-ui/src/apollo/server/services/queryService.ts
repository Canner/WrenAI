...
const normalizeSqlReferencesToManifest = (sql: string, manifest?: Manifest) => {
  ...
  return normalizeSqlColumnReferencesToManifest(normalizedSql, manifest);
};

const getSemanticColumnAlias = (
  column: string,
  validColumns: Set<string>,
): string | undefined => {
  const normalizedColumn = column.replace(/[^A-Za-z0-9]/g, '').toLowerCase();
  const candidatesByAlias: Record<string, string[]> = {
    pattern: ['name', 'category', 'description', 'id'],
    patterns: ['name', 'category', 'description', 'id'],
    failurename: ['name', 'category', 'description', 'id'],
    failurepattern: ['name', 'category', 'description', 'id'],
    failurepatterns: ['name', 'category', 'description', 'id'],
  };

  const candidates = candidatesByAlias[normalizedColumn] || [];
  for (const candidate of candidates) {
    for (const validColumn of validColumns) {
      const normalizedValidColumn = validColumn
        .replace(/[^A-Za-z0-9]/g, '')
        .toLowerCase();
      if (normalizedValidColumn === candidate) {
        return validColumn;
      }
    }
  }

  if (normalizedColumn.endsWith('s')) {
    const singularColumn = normalizedColumn.slice(0, -1);
    for (const validColumn of validColumns) {
      const normalizedValidColumn = validColumn
        .replace(/[^A-Za-z0-9]/g, '')
        .toLowerCase();
      if (normalizedValidColumn === singularColumn) {
        return validColumn;
      }
    }
  }
...
const normalizeSqlColumnReferencesToManifest = (
  sql: string,
  manifest?: Manifest,
) => {
  ...
  return sql.replace(
    qualifiedColumnPattern,
    (match, qualifierReference: string, columnReference: string) => {
      const qualifier = normalizeSqlIdentifier(qualifierReference).toLowerCase();
      const column = normalizeSqlIdentifier(columnReference);
      ...
      const alias = getSemanticColumnAlias(column, validColumns);
      if (!alias) {
        return match;
      }

      return `${qualifierReference}.${quoteSqlIdentifier(alias)}`;
    },
  );
};
