...
jest.mock('@server/utils/askContext', () => ({
  ...
  toAskRuntimeIdentity: (...args: any[]) => mockToAskRuntimeIdentity(...args),
}));
...
describe('pages/api/v1/ask', () => {
  ...
  beforeEach(() => {
    ...
    mockToAskRuntimeIdentity.mockImplementation(
  ...
  it('uses derived runtime project for multi-source ask execution', async () => {
    const handler = (await import('../../pages/api/v1/ask')).default;
    const req = createReq();
    const res = createRes();
    ...
    expect(mockToAskRuntimeIdentity).toHaveBeenCalledWith(
