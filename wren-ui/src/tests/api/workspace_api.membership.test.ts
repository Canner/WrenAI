...
describe('workspace api routes', () => {
  ...
  it('submits a pending application for request-only workspaces', async () => {
    const handler = (await import('../../pages/api/v1/workspace/apply'))
      .default;
    const req = createReq({
      method: 'POST',
  ...
  it('activates invited memberships through join route', async () => {
    const handler = (await import('../../pages/api/v1/workspace/join')).default;
    const req = createReq({
      method: 'POST',
      body: { workspaceId: 'workspace-3' },
    });
  ...
  it('invites a member by email for managers', async () => {
    const handler = (await import('../../pages/api/v1/workspace/members/index'))
      .default;
    const req = createReq({
      method: 'POST',
  ...
  it('creates a workspace for platform admins', async () => {
    const handler = (await import('../../pages/api/v1/workspace')).default;
    const req = createReq({
      method: 'POST',
  ...
  it('updates default workspace preference', async () => {
    const handler = (await import('../../pages/api/v1/workspace/preferences'))
      .default;
    const req = createReq({
      method: 'PATCH',
  ...
  it('approves a pending member request', async () => {
    const handler = (await import('../../pages/api/v1/workspace/members/[id]'))
      .default;
    const req = createReq({
      method: 'PATCH',
