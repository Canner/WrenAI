...
const mockCreateApiHistoryRecord = jest.fn();
...
const mockSendSSEEvent = jest.fn();
const mockSendMessageStart = jest.fn();
const mockSendStateUpdate = jest.fn();
const mockSendError = jest.fn();
const mockEndStream = jest.fn();
const mockGetSqlGenerationState = jest.fn(() => 'SQL_GENERATION_DONE');
...
jest.mock('@server/utils', () => ({
  ...
  sendStateUpdate: mockSendStateUpdate,
  sendError: mockSendError,
  getSqlGenerationState: mockGetSqlGenerationState,
...
describe('pages/api/v1/stream/ask', () => {
  const runtimeScope = {
    project: null,
    selector: {
  ...
  beforeEach(() => {
    ...
    mockGetAskResult.mockResolvedValue({
      status: 'finished',
      type: 'TEXT_TO_SQL',
      response: [{ sql: 'select revenue from federated_sales' }],
      error: null,
      rephrasedQuestion: null,
      intentReasoning: null,
      sqlGenerationReasoning: null,
      retrievedTables: ['federated_sales'],
      traceId: 'trace-1',
    });
    ...
    mockStreamTextBasedAnswer.mockImplementation(async () => {
      ...
      process.nextTick(() => {
        emitter.emit('data', Buffer.from('data: {"message":"流式总结"}'));
        emitter.emit('end');
      });
      return emitter;
    });
  ...
  it('streams ask results using the derived federated runtime project', async () => {
    const handler = (await import('../../pages/api/v1/stream/ask')).default;
    const req = createReq();
    const res = createRes();
    ...
    expect(mockCreateApiHistoryRecord).toHaveBeenCalledWith(
      expect.objectContaining({
        runtimeScope,
        responsePayload: expect.objectContaining({
          sql: 'select revenue from federated_sales',
          summary: '流式总结',
        }),
      }),
    );
