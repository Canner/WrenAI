...
- **Validation boundaries — validate once, at the edge.** The MDL manifest's schema is enforced in exactly one place: `wren-core`'s serde deserialization, reached via `ManifestExtractor` and `SessionContext`. A manifest with a non-object entry in `models`/`views`, a missing `name`, or a non-string `name` is rejected there. **Downstream of a successful deserialization the schema holds**, so per-callsite `isinstance` guards over `manifest["models"]` / `["views"]` / `["columns"]` on those paths are dead code and are not accepted. **Upstream of it the opposite applies**: code that decodes raw manifest JSON and dereferences it before `wren-core` ever sees it (`context.validate_manifest` reads `manifest["views"]` before `WrenEngine` is even constructed), hand-edited project YAML, and external API responses are genuine input boundaries — validate there, and report the problem rather than crashing. Say which side of that line your change sits on.
