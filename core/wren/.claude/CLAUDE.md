...
- `wren cloud login|link|create|unlink|logout` — Bind a local project to a Wren Cloud project's git remote. The binding *is* the git remote; after binding, plain `git push`/`git pull` are the commands
...
- **Validation boundaries — validate once, at the edge.** The MDL manifest's schema is enforced in exactly one place: `wren-core`'s serde deserialization, reached via `ManifestExtractor` and `SessionContext`. A manifest with a non-object entry in `models`/`views`, a missing `name`, or a non-string `name` is rejected there. **Downstream of a successful deserialization the schema holds**, so per-callsite `isinstance` guards over `manifest["models"]` / `["views"]` / `["columns"]` on those paths are dead code and are not accepted. **Upstream of it the opposite applies**: code that dereferences a structure before `wren-core` ever sees it is at a genuine input boundary and should validate and report rather than crash. `context.convert_mdl_to_project` is the clearest case — `wren context init --from-mdl` hands it a user-supplied JSON file verbatim. Hand-edited project YAML and external API responses are the same shape. Say which side of that line your change sits on.
