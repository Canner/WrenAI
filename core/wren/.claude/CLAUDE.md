...
- **Validation boundaries — validate once, at the edge.** The MDL manifest's schema is enforced in exactly one place: `wren-core`'s serde deserialization, reached via `ManifestExtractor` and `SessionContext`. A manifest with a non-object entry in `models`/`views`, a missing `name`, or a non-string `name` is rejected there. By the time Python reads a manifest dict the schema already holds, so per-callsite `isinstance` guards over `manifest["models"]` / `["views"]` / `["columns"]` are dead code and are not accepted. Genuine input boundaries are the opposite case — raw user JSON before it reaches `wren-core`, hand-edited project YAML, external API responses — and there the code should validate and report the problem rather than crash.
